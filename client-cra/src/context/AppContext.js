...
export const AppProvider = ({ children }) => {
  ...
  async function getReserves(contractAddress, contractABI, signer, user) {
    const provider = new ethers.providers.Web3Provider(window.ethereum)
    const signe = provider.getSigner()
    const contract = new ethers.Contract(contractAddress, contractABI, signe)
  ...
  async function addLiquidity(
    ...
    console.log(
      'Convert A: ',
      BigNumber.from(convertA).toString(),
      'Convert B: ',
      BigNumber.from(convertB).toString(),
    )
    let statics = await contract.callStatic
      ...
      .then(async (res) => {
        let transaction
        console.log('Convert A: ', convertA, 'Convert B: ', convertB)
        try {
  ...
  async function participation(
    ...
    let convertPrediction = ethers.utils.parseUnits(
      userPrediction.toString(),
      18,
    )
    await contract.callStatic
      .bet(data.id, convertPrediction, data.amount, {
        gasLimit: 500000,
      })
      .then(async (res) => {
        let transaction
        try {
          let part = await contract.bet(
            data.id,
            convertPrediction,
            data.amount,
            {
          ...
          swal({
            ...
            icon: 'success',
            title: `Bet joined `,
            text: `You just joined a bet, good luck`,
            className: 'text-center',
            button: false,
            footer: `<a target="_blank" href=${chainlist[0].explorer}/tx/${transaction}/>See Transaction</a>`,
          })
